...
def build_dataset(
    ...
    for x, y in stream:
        ...
        if tracker._reference_locked:
            baselines.update(correct, batch_idx)
...
def run_synthetic_sea_drift():
    """
    Fully controlled synthetic benchmark: SEA generator with an injected
    ABRUPT concept change at a known instance count -> exact ground truth.
    ...
    drift_at_instance = 5000  # exact, known switch point
    stream = synth.ConceptDriftStream(
        stream=synth.SEA(variant=0, seed=42),
        drift_stream=synth.SEA(variant=2, seed=42),
        seed=1,
        position=drift_at_instance,
        width=50,  # small = abrupt switch (must be >=~10 to avoid overflow); increase for gradual drift
    )
    feature_names = [0, 1, 2]  # SEA yields integer-keyed features, not named ones

    batch_size = 50
    reference_size = 300
    # convert instance-level drift point -> approx batch index
    drift_batch = max(0, (drift_at_instance - reference_size) // batch_size)
